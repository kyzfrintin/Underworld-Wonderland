...
func decide_attack(dis):
	var attacks : Dictionary = parent.agent.attack_dict
	var list = attacks.values()
	if !list.size() > 1:
		return [list[0].animation, parent.attack_enabled_array[0], 0]
	var a : String
	var d = parent.agent.max_attack_range + 1
	var can_attack
	var num
	for i in list:
		if i.a_range > dis and i.a_range < d:
			...
			num = list.find(i)
			a = i.get("animation")
			parent.agent.track_int = i.get("track_int")
			can_attack = parent.attack_enabled_array[num]
	return [a, can_attack, num]

func track_dis():
	dis = parent.agent.global_transform.origin.distance_to(parent.player.translation)
